...
echo "$($PSQL "
  SELECT w.name FROM games INNER JOIN teams AS w ON games.winner_id = w.team_id WHERE year=2014 AND round='Eighth-Final'
  UNION
  SELECT o.name FROM games INNER JOIN teams AS o ON games.opponent_id = o.team_id WHERE year=2014 AND round='Eighth-Final';
")"
